...
\begin{titlepage}
    ...
    \begin{tikzpicture}[remember picture, overlay]
        ...
        \node[anchor=north west] at ($(current page.north west)+(1.6cm,-1.8cm)$) {%
            \includegraphics[height=3cm]{logo-ipn-guinda.png}
        };

        % Líneas decorativas verticales (alineadas con la portada ejemplo)
        % Usamos current page para que funcione igual en letter
        \draw[line width=3pt, gold]
            ($(current page.north west)+(2.1cm,-5.2cm)$) --
            ($(current page.south west)+(2.1cm,5.2cm)$);

        \draw[line width=3pt, ipnBrown]
            ($(current page.north west)+(3.3cm,-5.2cm)$) --
            ($(current page.south west)+(3.3cm,5.2cm)$);

        % Logo UPIITA (abajo izquierda)
        \node[anchor=south west] at ($(current page.south west)+(1.4cm,2.0cm)$) {%
    ...
    % -------- SINODALES (con líneas) ----------
    \begin{tabular}{p{6cm}p{6cm}}
        \centering\rule{6cm}{0.4pt} & \centering\rule{6cm}{0.4pt} \tabularnewline
        \centering Huerta Trujillo Iliac & \centering Villordo Jimenez Iclia
    \end{tabular}
...
Las arquitecturas de \gls{iot} proporcionan marcos de referencia para estructurar sistemas distribuidos que abarcan dispositivos físicos, comunicación, procesamiento y aplicaciones. Su correcta selección e implementación es de gran importancia para asegurar requisitos no funcionales como escalabilidad, resiliencia y seguridad, especialmente en escenarios de telemetría energética y detección de consumos anómalos en tiempo real.
